...
- **3D ball extension (version 20)**: 9 fields in `serverparam.h` (members [serverparam.h:636-644](../../src/serverparam.h:636), accessors [serverparam.h:1012-1020](../../src/serverparam.h:1012)) registered via `addParam(...)` at version `20` ([serverparam.cpp:975-983](../../src/serverparam.cpp:975), defaults set [serverparam.cpp:1509-1517](../../src/serverparam.cpp:1509)): `2d_mode` (master gate, default `true`), `player_height` (also doubles as the max ball reach-height for kickability/heading — the former separate `player_reach_height` concept was merged into this one field), `goal_height`, `gravity` (default `0.1`, per-cycle, NOT real-world 9.8), `ball_bounce_restitution` (default `0.5` — scales the ball's ENTIRE velocity vector, vx/vy/vz, on every ground/post/crossbar bounce), `bounce_stop_speed`, `roll_stop_speed`, `height_power_cost` (default `0.25`), `precise_bounce_timing` (default `true`). **Reworked 2026-07-10** (matching the `3d-kick-lab/physics.js` sandbox's same-day simplification): `ball_bounce_friction`, `loft_power_cost`, and `air_decay` were REMOVED entirely — bounce friction is now folded into the single `ball_bounce_restitution` coefficient, a kick's horizontal/vertical split is pure geometry (`cos(loft)`/`sin(loft)`, no loft power penalty), and the ball has zero horizontal friction while airborne (ground-only `ball_decay` applies once `pos_z<=0`). Formulas are hand-ported from the sibling `3d-kick-lab/physics.js` sandbox (see that repo's copilot-instructions.md) — not a merge/shared-code relationship.
