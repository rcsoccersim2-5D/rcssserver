...
- **3D ball extension (version 20)**: `kick`/`long_kick` widened to a 3-arg form (`kick(power, dir, loft = 0.0)`), and a new pure-virtual `chest_trap()` added to `Builder` (renamed from `stop_ball()`) — both in `pcombuilder.h`. `Player::kick`/`long_kick` overrides (player.cpp) forward the real `loft` into the private `kickImpl()`/`longKickImpl()` helpers (see entities instructions); the `loft=0.0` default keeps every pre-v20 2-arg call site behaviorally unchanged. `Player::chest_trap()` calls the new `Stadium::chestTrap()` (renamed from `Stadium::stopBall()`, mirrors `Stadium::moveBall()`) and silently no-ops (same reject pattern as `Player::move()`) when `ServerParam::is2dMode()==true` or the ball isn't currently kickable — no new fault/error bit was added. It additionally calls `Stadium::kickTaken( *this, PVector(0,0), 0.0 )` right after `Stadium::chestTrap()`, reusing the exact same referee fan-out dispatch a real kick uses so an offside-positioned player is marked via `OffsideRef::kickTaken()` just like a kick — see referee-rules.instructions.md.
- **`chest_trap` grammar** (renamed from `stop_ball`): new token `RCSS_PCOM_CHEST_TRAP` ([player_command_parser.ypp:78](../../src/player_command_parser.ypp:78)) and rule `chest_trap_com : RCSS_PCOM_LP RCSS_PCOM_CHEST_TRAP RCSS_PCOM_RP` ([player_command_parser.ypp:232-238](../../src/player_command_parser.ypp:232)), wired into the `command` alternation via `| chest_trap_com` ([player_command_parser.ypp:141](../../src/player_command_parser.ypp:141)) — a 4-layer chain (lexer keyword → token → grammar rule → `BUILDER.chest_trap()` call) matching the "4 files in lockstep" pattern in Important Notes below. `kick`/`long_kick`'s 3-float alternative follows the existing multi-arity pattern already used by `dash_com` (see Patterns & Conventions above).
