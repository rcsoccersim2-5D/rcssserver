...
- **3D ball extension (version 20)**: `kick`/`long_kick` widened to a 3-arg form (`kick(power, dir, loft = 0.0)`), and a new pure-virtual `stop_ball()` added to `Builder` — both in `pcombuilder.h`. `Player::kick`/`long_kick` overrides (player.cpp) forward the real `loft` into the private `kickImpl()`/`longKickImpl()` helpers (see entities instructions); the `loft=0.0` default keeps every pre-v20 2-arg call site behaviorally unchanged. `Player::stop_ball()` calls the new `Stadium::stopBall()` (mirrors `Stadium::moveBall()`) and silently no-ops (same reject pattern as `Player::move()`) when `ServerParam::is2dMode()==true` or the ball isn't currently kickable — no new fault/error bit was added.
- **`stop_ball` grammar**: new token `RCSS_PCOM_STOP_BALL` ([player_command_parser.ypp:78](../../src/player_command_parser.ypp:78)) and rule `stop_ball_com : RCSS_PCOM_LP RCSS_PCOM_STOP_BALL RCSS_PCOM_RP` ([player_command_parser.ypp:232-234](../../src/player_command_parser.ypp:232)), wired into the `command` alternation via `| stop_ball_com` ([player_command_parser.ypp:141](../../src/player_command_parser.ypp:141)) — a 4-layer chain (lexer keyword → token → grammar rule → `BUILDER.stop_ball()` call) matching the "4 files in lockstep" pattern in Important Notes below. `kick`/`long_kick`'s 3-float alternative follows the existing multi-arity pattern already used by `dash_com` (see Patterns & Conventions above).
