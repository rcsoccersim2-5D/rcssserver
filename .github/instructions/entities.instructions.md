...
- **3D ball extension — Ball-only vertical physics (version 20, gated by `!ServerParam::is2dMode()`)**: `Ball` alone gets `M_pos_z`/`M_vel_z`/`M_accel_z` ([object.h:640-647](../../src/object.h:640)) plus `incZ()` ([object.cpp:776](../../src/object.cpp:776)), `checkPostAndCrossbar3D()` ([object.cpp:728](../../src/object.cpp:728)), `bounceSettleThreshold()`, and `applyBounceFriction()` ([object.h:712-734](../../src/object.h:712)) — `Player` gets NO z-position (players stay 2D by design). `MPObject::_inc()` itself is untouched/byte-identical; `Stadium::incMovableObjects()` ([stadium.cpp:913-933](../../src/stadium.cpp:913)) calls `_inc()` for every movable object as before, then conditionally calls `M_ball->incZ()` only when `!ServerParam::instance().is2dMode()`. `Player::kick`/`long_kick` keep their original 2-arg public signatures (required for `pcom::Builder` override compatibility — see player-command-protocol instructions); the new loft/height-cost logic lives in private `Player::kickImpl()`/`longKickImpl(power, dir, loft)` helpers (player.cpp) instead.
- **Height-gated goalie catch**: `Player::goalieCatch()` ([player.cpp:1623](../../src/player.cpp:1623)) adds a short-circuited clause — `! max_catchable.inArea(rotated_pos) || ( ! SP.is2dMode() && M_stadium.ball().posZ() > SP.playerHeight() )` ([player.cpp:1734-1735](../../src/player.cpp:1734)) — so a ball above `player_height()` cannot be caught when 3D mode is active; provably a no-op when `is2dMode()==true` via short-circuit evaluation.
