...
## Important Notes
- **3D ball extension — `REC_VERSION_7`** ([types.h:208](../../src/types.h:208)): new game-log version recording ball z, added alongside a comment explicitly noting `DEFAULT_REC_VERSION` intentionally stays at `REC_VERSION_6` ([types.h:210-213](../../src/types.h:210)) — existing deployments keep logging in the legacy format unless an operator explicitly opts in, consistent with `2d_mode` defaulting to legacy-safe. New `InitSenderLoggerV7` ([initsenderlogger.cpp:517-555](../../src/initsenderlogger.cpp:517), `autoReg`'d at 7, [initsenderlogger.cpp:664](../../src/initsenderlogger.cpp:664)); no new `DispSenderLoggerV7` class was needed — the existing version-agnostic `DispSenderLoggerV4` (delegates to whatever `SerializerMonitor` it's given) is simply re-registered at version 7 too ([dispsender.cpp:1164](../../src/dispsender.cpp:1164)), the same way v5/v6 already reuse it. `Logger::setSenders()`'s `monitor_version = log_version - 1` mapping ([logger.cpp:164-167](../../src/logger.cpp:164)) needed zero code changes — it auto-maps `REC_VERSION_7 → monitor_version 6 → SerializerMonitorStdv6` (see monitor-protocol instructions) purely from the existing arithmetic. `csvsaver.cpp` was confirmed to have zero `REC_VERSION`/`gameLogVersion` references — no change needed there.
