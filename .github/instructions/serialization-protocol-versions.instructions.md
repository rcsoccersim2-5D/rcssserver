...
- **Concrete per-version classes** form a **linear inheritance chain per family** (confirmed via grep of all `serializer*.h`):
  - Player: `Stdv1 → Stdv5 → Stdv7 → Stdv8 → Stdv13 → Stdv14 → Stdv18 → Stdv20`
  - Coach: `Stdv1 → Stdv7 → Stdv8 → Stdv13 → Stdv14 → Stdv20`
...
- **3D ball extension (version 20)** — new classes (each in its own file, per-version-file convention): `SerializerPlayerStdv20` (`serializerplayerstdv20.h/.cpp`, subclasses `SerializerPlayerStdv18`) overrides the new `serializeFSBall3D(...)` virtual (fullstate ball z/vel_z, `serializer.h`) plus 2 new `serializeVisualObject` overloads carrying a trailing `elevation` field for the `(see)` stream (elevation = `atan2(posZ, dist)`, computed in `sendHighBall`, `visualsenderplayer.cpp`). `SerializerCoachStdv20` (`serializercoachstdv20.h/.cpp`, subclasses `SerializerCoachStdv14`) mirrors this with a coach-side z overload (raw/non-noised, since the coach `look` channel is unnoised — `visualsendercoach.cpp`). Both `autoReg`'d at version `20` ([serializerplayerstdv20.cpp:113](../../src/serializerplayerstdv20.cpp:113), [serializercoachstdv20.cpp:79](../../src/serializercoachstdv20.cpp:79)) — confirmed non-colliding (prior max was 19 for both families). `SerializerOnlineCoachStdv20` was confirmed NOT needed: `SerializerOnlineCoach` already delegates non-virtually to whatever concrete `SerializerCoach` was negotiated. All 3 new elevation/z overloads are **virtual with legacy-reproducing no-op defaults** on the base classes — sender `.cpp` files call them unconditionally, so pre-v20 clients get byte-identical output via ordinary virtual dispatch (no runtime `version() >= 20` checks anywhere in the sender code).
- The fullstate path (`FullStateSenderPlayerV20`, `fullstatesender.h/.cpp`, subclasses `FullStateSenderPlayerV18`) is a **`senders-dispatch`-instruction** class, not a serializer, but is closely coupled here: it overrides `sendBall()` to call the new `serializeFSBall3D(...)`, autoReg'd at version 20 ([fullstatesender.cpp:583](../../src/fullstatesender.cpp:583)). Coach has **no fullstate/sendBall path at all** (fullstate is player-only) — confirmed by grep, no coach-side fullstate change was needed.
