...
- **3D ball extension (version 20)** — `SerializerPlayerStdv20` (`serializerplayerstdv20.h/.cpp`, subclass of `SerializerPlayerStdv18`) overrides `serializeFSBall3D(...)` for fullstate plus three ball-only normal-see virtual shapes: low quality emits `dir z`, high quality without planar changes emits `dist dir z`, and high quality with changes emits `dist dir dist_chg dir_chg z vz`. The visual sender passes raw `Ball::posZ()` and, only beside `dist_chg/dir_chg`, raw `Ball::velZ()`; neither vertical value uses visual noise. `SerializerPlayer` supplies legacy-reproducing default bodies that drop z/vz, so protocols 1-19 retain their old layouts through ordinary virtual dispatch with no sender-side version check. Separately, `SerializerCoachStdv20` (`serializercoachstdv20.h/.cpp`, subclass of `SerializerCoachStdv14`) keeps the existing coach-side raw-z overload for unnoised `(look)` output; this player change does not add coach vz. Both concrete serializers are `autoReg`'d at version `20`. `SerializerOnlineCoachStdv20` is unnecessary because `SerializerOnlineCoach` delegates to the negotiated concrete `SerializerCoach`.
