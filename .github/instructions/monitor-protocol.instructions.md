...
- **Version registration** (serializer side, [serializermonitor.cpp:850-855](../../src/serializermonitor.cpp:850)): `v1`,`v2`→`SerializerMonitorStdv1`; `v3`→`SerializerMonitorStdv3`; `v4`→`SerializerMonitorStdv4` (adds stamina capacity); `v5`→`SerializerMonitorStdv5` (adds focus point); `v6`→`SerializerMonitorStdv6` (3D ball extension — appends ball z, see below); `-1`→`SerializerMonitorJSON`. Each serializer internally binds to a specific `SerializerCommon` version too (v3→common 8, v4→common 15, v5→common 18, JSON→common -1) — see [serializermonitor.cpp:63,113,336,380,441](../../src/serializermonitor.cpp:63).
- **3D ball extension — `SerializerMonitorStdv6`** (`serializermonitorstdv6.h/.cpp`, subclasses `SerializerMonitorStdv5`): overrides `serializeBall` to append the ball's quantized `posZ()`, `autoReg`'d at version `6` ([serializermonitorstdv6.cpp:76](../../src/serializermonitorstdv6.cpp:76)) — confirmed non-colliding (prior max was 5). `SerializerMonitorJSON::serializeBall` was updated in-place for parity (appends z as an extra JSON field) rather than via a version chain, since JSON has no numeric version slots. The inheritance chain was audited when adding this: `SerializerMonitorStdv5`/`v4`/`v3` all already carry a real (non-empty, inherited from `Stdv3`) `serializeBall` body — the empty no-op default in `serializer.h` is pre-existing dead code only reachable by legacy v1/v2, and was NOT touched by this change.
