...
- The formulas underlying `gravity`/`ball_bounce_restitution`/`loft_power_cost`/etc. were hand-ported (not shared/merged code) from the `3d-kick-lab` sandbox's `physics.js` — see that repo's own copilot-instructions.md, which now documents its role as historical/reference now that porting is complete.

## See Also
- [config-params.instructions.md](config-params.instructions.md) — the 12 new `ServerParam` fields (`2d_mode`, `gravity`, etc.) and their defaults.
