...
- **3D ball flight is now an opt-in extension** (protocol version 20): `2d_mode` (default `true`) keeps physics legacy-equivalent and keeps protocols 1-19 wire-identical; a v20 client always receives its vertical fields, which are zero in 2D mode. Setting `2d_mode=false` enables ball-only vertical physics (loft kicks, gravity, bouncing, height-aware goalie catch). See [3d-protocol-migration.instructions.md](instructions/3d-protocol-migration.instructions.md) for the exact player/client wire contract.
...
|-------------|------------------|---------|------|
| Config & Parameter Handling | `src/serverparam.*, src/playerparam.*, src/heteroplayer.*, rcss/conf/**` | Generic `rcss::conf::Builder` framework backing `ServerParam`/`PlayerParam`; heterogeneous player-type sampling; now includes 9 version-20 3D-ball-extension params (`2d_mode`, `gravity`, `player_height`, `goal_height`, etc. — reworked 2026-07-10: `air_decay`/`loft_power_cost`/`ball_bounce_friction` removed, merged into a single `ball_bounce_restitution`) | [config-params.instructions.md](instructions/config-params.instructions.md) |
...
| Sensory Message Serialization & Protocol Versions | `src/serializer*.cpp, src/serializer*.h` | Versioned `Serializer*Stdv{N}` subclass chains selected via per-connection `Factory` lookup; player v20 adds raw ball z/vz to normal see and z/vz to fullstate, while coach v20 keeps its separate raw-z extension | [serialization-protocol-versions.instructions.md](instructions/serialization-protocol-versions.instructions.md) |
...
| 3D Protocol Migration Guide | External-consumer facing (agent teams, `rcssmonitor`) | Exact v20 player-see `dir z` / `dist dir z` / `dist dir dist_chg dir_chg z vz` layouts, parser rules, commands, 2D-mode zero semantics, and monitor pointers | [3d-protocol-migration.instructions.md](instructions/3d-protocol-migration.instructions.md) |
