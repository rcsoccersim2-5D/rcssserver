...
| 3D Protocol Migration Guide | External-consumer facing (agent teams, `rcssmonitor`) | How to opt into the version-20 3D ball extension: new `kick`/`long_kick` 3-arg form, `chest_trap` command (renamed from `stop_ball`, now offside-enforced via the kick-style referee dispatch), `2d_mode` client-side semantics, monitor rendering pointers | [3d-protocol-migration.instructions.md](instructions/3d-protocol-migration.instructions.md) |
